...
class evo_settings_settings{
		...
		return apply_filters('eventon_settings_tab1_arr_content', array(
			...
			,array(
				...
				'fields'=>array(
					...
					array('id'=>'evo_ftimghover','type'=>'yesno','name'=>__('Disable hover effect on featured image','eventon'),'legend'=>'Remove the hover moving animation effect from featured image on event.'),
					array('id'=>'evo_ftimgclick','type'=>'yesno','name'=>__('Disable zoom effect on click','eventon'),'legend'=>'Remove the moving animation effect from featured image on click event.'),
					/*array('id'=>'evo_ftimg_fullheight','type'=>'yesno','name'=>'Show featured image at 100% height', ), -- removed 2.2.20*/

					array('id'=>'evo_ftimgheight','type'=>'text','name'=>__('Minimize height for featured image (value in pixels)','eventon'), 'default'=>'eg. 400'),
					array('id'=>'evo_ftimg_height_sty','type'=>'dropdown','name'=>__('Featured image display style (On event load)','eventon'), 'legend'=>'Select which display style you want to show the featured image on event card when event first load',
						'options'=> array(
					...
					array('id'=>'evo_ftim_mag','type'=>'yesno','name'=>__('Show magnifying glass over featured image','eventon'),'legend'=>'This will convert the mouse cursor to a magnifying glass when hover over featured image. <br/><br/><img src="'.AJDE_EVCAL_URL.'/assets/images/admin/cursor_mag.jpg"/>'),
				...
				'fields'=>array(
					...
					array('id'=>'evcal_sdd','type'=>'note','name'=>__('<i>NOTE: As we have built our own <a href="http://www.myeventon.com/addons/event-tickets/" target="_blank">event Ticket addon</a> we are no longer adding additional features for eventbrite. Financially Event Ticket addon is a better option as you will NOT have to pay percentage of each ticket sale with EventTicket addon.</i>','eventon')),
					array('id'=>'evcal_evb_events','type'=>'yesno','name'=>__('Enable EventBrite data fetching for calendar events','eventon'),'legend'=>'Once enabled, this will allow you to connect eventbrite to event calendar and populate event data such as event name, event location, ticket price for events, event capacity & link to buy ticket','afterstatement'=>'evcal_evb_events'),
					array('id'=>'evcal_evb_events','type'=>'begin_afterstatement'),
					array('id'=>'evcal_evb_api','type'=>'text','name'=>__('EventBrite API Key','eventon')),
					...
					// meetup
					array('id'=>'evcal_s','type'=>'subheader','name'=>__('Meetup','eventon')),
					array('id'=>'evcal_api_meetup','type'=>'yesno','name'=>__('Enable Meetup data fetching for calendar events','eventon'),'legend'=>'Once enabled, this will allow your to connect meetup events and populate calendar events with meetup event data such as event name, event time, location, & meetup event url','afterstatement'=>'evcal_api_meetup'),
					array('id'=>'evcal_api_meetup','type'=>'begin_afterstatement'),
					array('id'=>'evcal_api_mu_key','type'=>'text','name'=>__('Meetup API Key','eventon')),
	...
	// rearrange fields
		...
		function custom_meta_fields(){
			...
			$cmf_addition_x= array(array('id'=>'evcal__note','type'=>'note','name'=>__('You can add upto 3 additional custom meta fields for each event using the below fields. <br/><b>NOTE: </b>Once new data field is activated go to <b>myEventon> Settings> EventCard</b> and rearrange the order of this new field and save changes for it to show on front-end. <br/>(* Required values)','eventon')),
