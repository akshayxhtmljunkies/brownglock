...
maxAdmin.prototype.init = function () {
 	...
 	//	$('.maxbutton_thickbox').on('click', this.fixThickSize);
		...
		$(document).on('click', '[data-form]', $.proxy(this.buttonSubmit, this)); // remove save buttons ( outside form )
...
maxAdmin.prototype.clickAddButton = function (e, callback) 
{
	e.preventDefault();
	e.stopPropagation(); 
	...
	$(document).on('click','.pagination span', function (e)  // eventception
	{
		...
		self.loadPostEditScreen(page, callback); 
	}) ; 
	
	this.loadPostEditScreen(0, callback);
}

// Callback is the add function on button select
maxAdmin.prototype.loadPostEditScreen = function(page, callback)
{
